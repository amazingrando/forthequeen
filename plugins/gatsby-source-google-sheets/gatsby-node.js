...
exports.sourceNodes = async (
  ...
  if (!spreadsheetId) {
    throw new Error(
      'spreadsheetId is required for gatsby-source-google-sheets',
    );
  }

  if (!credentials) {
    console.log(
      '⚠️  No Google credentials provided. Skipping Google Sheets data fetch.',
    );
    console.log('📋 To enable Google Sheets integration:');
    console.log(
      '   1. Set GOOGLE_SERVICE_ACCOUNT_CREDENTIALS environment variable',
    );
  ...
  try {
    ...
    if (credentials.type === 'service_account') {
      ...
      console.log(
        '⚠️  OAuth 2.0 credentials detected. For production use, consider using service account credentials.',
      );
      ...
      console.log(
        '   4. Replace your credentials.json with the service account JSON',
      );

      // For now, we'll use a simple API key approach or throw an error
      throw new Error(
        'OAuth 2.0 credentials are not supported. Please use service account credentials instead.',
      );
    } else {
      throw new Error(
        'Invalid credentials format. Please use service account credentials.',
      );
    }
    ...
    console.log(
      `Successfully fetched ${rows.length - 1} rows from Google Sheets`,
    );
  } catch (error) {
